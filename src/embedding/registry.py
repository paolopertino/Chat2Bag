...
def create_embedder(config, device: str = "cpu") -> FrameEmbedder:
    ...
    `device` is passed into the backend before constructor-time probes run.
    """
    ...
    return _REGISTRY[key](config, device=device)

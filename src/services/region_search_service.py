...
class RegionSearchService:
    ...
    def search_by_text(self, text: str, bag_paths: list[str], top_k: int, area: dict | None = None) -> list[dict]:
        self._require_bags(bag_paths)
        if not text.strip():
            raise ValueError("Text query must not be empty.")
        return self._searcher.search_by_text(text=text, bag_paths=bag_paths, top_k=top_k, area=area)

    def search_by_frame(self, support_file_path: str, points: list[dict], bag_paths: list[str], top_k: int, area: dict | None = None) -> list[dict]:
        self._require_bags(bag_paths)
        ...
        return self._searcher.search_by_points(
            ...
            exclude_file_path=support_file_path, area=area,
        )

    def search_by_image(self, image_bytes: bytes, points: list[dict], bag_paths: list[str], top_k: int, area: dict | None = None) -> list[dict]:
        self._require_bags(bag_paths)
        if not image_bytes:
            raise ValueError("Image payload is empty.")
        image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        return self._searcher.search_by_points(
            image=image, points=points, bag_paths=bag_paths, top_k=top_k, area=area,
        )
